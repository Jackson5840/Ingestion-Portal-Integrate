...
def getDuplicatesfordata():
    ...
    global g_datadict
    if "g_datadict" not in globals():
        g_datadict = sis.datamgmt.generateMtrxFrData()
    result = sis.datamgmt.checkduplicatesinternal(
        g_datadict["datarows"],
        datadict,
        g_datadict["neuron_names"],
    )
    return jsonify(result)
...
if os.getenv('SIS_INIT_ON_STARTUP', '1') == '1':
    init_faiss(0)
