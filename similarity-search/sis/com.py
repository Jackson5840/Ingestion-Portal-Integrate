...
def mysql_connect(database=None, host=None, port=None):
    kwargs = {
        "user": cfg.dbuser,
        "password": cfg.dbpass,
        "host": host or cfg.dbhost,
        "database": database or cfg.dbsel,
        "auth_plugin": cfg.db_auth_plugin,
    }
    if port is not None:
        kwargs["port"] = port
    if cfg.dbtimeout:
        kwargs["connect_timeout"] = cfg.dbtimeout
    return mysc.connect(**kwargs)
    ...
    def myconnect_(*args, **kwargs):
        ...
        if sshswitch:
            ...
            with SSHTunnelForwarder(
                ...
                conn = mysql_connect(host='127.0.0.1', port=tunnel.local_bind_port)
                conn.autocommit = True
                try:
                    rv = f(conn, *args, **kwargs)
                except Exception:
